...
/**
 * Leitor STORE e DEFLATE. Ignora diretórios e __MACOSX/.
...
export async function lerZip(bytes) {
  ...
  for (let i = 0; i < quantidade; i++) {
    ...
    const nome = decodificador.decode(b.subarray(pos + 46, pos + 46 + tamanhoNome));
    ...
    const dados = b.slice(inicio, inicio + tamanhoComprimido);
    if (metodo === 0) saida.set(nome, dados);
    else if (metodo === 8) saida.set(nome, await inflarRaw(dados));
